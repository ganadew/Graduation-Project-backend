...
class Window(QDialog, ui.Ui_Dialog):
    def __init__(self):
        ...
        self.token = ''
        # 맥북 내장 카메라 우선 (아이폰 연속성 카메라 대신)
        cam_idx = 1 if platform.system() == "Darwin" else 0
        self.camera = cv.VideoCapture(cam_idx, cv.CAP_AVFOUNDATION if platform.system() == "Darwin" else cv.CAP_ANY)
