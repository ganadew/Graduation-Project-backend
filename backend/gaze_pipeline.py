...
CALIBRATION_SAMPLE_DURATION_SEC = 10.0
...
CALIBRATION_MAX_DURATION_SEC = 11.0    # 분산이 높으면 최대 이 시간까지 더 수집
...
def get_pupil_position(landmarks):
    """
    MediaPipe iris landmark를 사용해 양쪽 홍채 중심을 구하고,
    눈 기준 상대좌표(0~1)로 반환.
    Returns: (pupil_x_rel, pupil_y_rel) or None
    """
    if len(landmarks) < 478:
        return None

    # 양쪽 iris 중심 계산
    left_iris = np.array([[landmarks[i].x, landmarks[i].y] for i in LEFT_IRIS], dtype=np.float64)
    right_iris = np.array([[landmarks[i].x, landmarks[i].y] for i in RIGHT_IRIS], dtype=np.float64)
    ...
    l_top = landmarks[LEFT_EYE["top"]]
    l_bottom = landmarks[LEFT_EYE["bottom"]]

    # 오른쪽 눈 윤곽
    r_outer = landmarks[RIGHT_EYE["outer"]]
    r_inner = landmarks[RIGHT_EYE["inner"]]
    r_top = landmarks[RIGHT_EYE["top"]]
    r_bottom = landmarks[RIGHT_EYE["bottom"]]

    # 왼쪽 눈 기준 상대좌표
    left_x_rel = (left_center[0] - l_outer.x) / ((l_inner.x - l_outer.x) + 1e-12)
    left_y_rel = (left_center[1] - l_top.y) / ((l_bottom.y - l_top.y) + 1e-12)

    # 오른쪽 눈 기준 상대좌표
    right_x_rel = (right_center[0] - r_outer.x) / ((r_inner.x - r_outer.x) + 1e-12)
    right_y_rel = (right_center[1] - r_top.y) / ((r_bottom.y - r_top.y) + 1e-12)

    # clamp
    left_x_rel = max(0.0, min(1.0, left_x_rel))
    left_y_rel = max(0.0, min(1.0, left_y_rel))
    right_x_rel = max(0.0, min(1.0, right_x_rel))
    right_y_rel = max(0.0, min(1.0, right_y_rel))

    # 양쪽 눈 평균
    pupil_x_rel = (left_x_rel + right_x_rel) / 2.0
    pupil_y_rel = (left_y_rel + right_y_rel) / 2.0

    return (pupil_x_rel, pupil_y_rel)
